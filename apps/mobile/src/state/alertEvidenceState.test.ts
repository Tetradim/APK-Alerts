...
test("alert test evidence summary blocks physical proof without signal capture evidence", () => {
  const physicalSignal = normalizeBridgeSignalEvent({
    event_id: "bus-physical-missing-capture",
    event_type: "signal.observed",
    source_bot: "chrome-discord-bridge",
    created_at: "2026-06-27T17:00:00.000Z",
    correlation_id: "chrome-message-missing-capture",
    payload: {
      contract_version: CHROME_DISCORD_MESSAGE_CONTRACT_VERSION,
      event_id: "chrome-message-missing-capture",
      channel_id: "chrome-alerts",
      channel_name: "chrome-alerts",
      channel_url: "https://discord.com/channels/1/chrome-alerts",
      author_id: "mike",
      ...
      ingestion_result: {
        status: "accepted",
        alert_inserted: true,
        alert_id: "alert-missing-capture",
        trade_requested: true,
        trade_request_reason: "risk approved; order intent queued",
        skip_reason: "",
      },
  ...
  const auditedDecision = normalizeBridgeAlertDecisionEvent({
    id: "audit-physical-missing-capture",
    category: "alert_ingestion",
    action: "bridge_alert_decision",
    summary: "Chrome bridge alert accepted and queued.",
    severity: "info",
    ...
    details: {
      ...
      author: { id: "mike", name: "MikeInvesting" },
      raw_text: "BTO SPY 500C 6/21 @ 1.25",
      capture_path: "C:/captures/chrome-message-missing-capture.json",
      parsed: { ticker: "SPY" },
      parser: { confidence: "high" },
      source: {
        key: "chrome-alerts",
  ...
  assert.equal(summary.parserLabel, "Parser proof high");
  assert.equal(summary.queueLabel, "Order request queued");
  assert.equal(summary.auditLabel, "Audit audit-physical-missing-capture");
...
test("alert test evidence summary blocks physical parsed payload mismatch", () => {
  const physicalSignal = normalizeBridgeSignalEvent({
    event_id: "bus-parsed-payload-mismatch",
    event_type: "signal.observed",
    source_bot: "chrome-discord-bridge",
    created_at: "2026-06-27T17:00:00.000Z",
    correlation_id: "chrome-message-parsed-payload-mismatch",
    payload: {
      contract_version: CHROME_DISCORD_MESSAGE_CONTRACT_VERSION,
      event_id: "chrome-message-parsed-payload-mismatch",
      channel_id: "chrome-alerts",
      channel_name: "chrome-alerts",
      author_id: "mike",
      author_name: "MikeInvesting",
      raw_text: "BTO SPY 500C 6/21 @ 1.25",
      parsed: { ticker: "SPY", action: "BTO" },
      parser_metadata: { confidence: "high" },
      ingestion_result: {
        status: "accepted",
        alert_inserted: true,
        alert_id: "alert-parsed-payload-mismatch",
        trade_requested: true,
        trade_request_reason: "risk approved; order intent queued",
        skip_reason: "",
      },
  ...
  const auditedDecision = normalizeBridgeAlertDecisionEvent({
    id: "audit-parsed-payload-mismatch",
    category: "alert_ingestion",
    action: "bridge_alert_decision",
    summary: "Chrome bridge alert accepted and queued.",
    severity: "info",
  ...
  assert.equal(summary.queueLabel, "Order request queued");
  assert.equal(summary.auditLabel, "Audit audit-parsed-payload-mismatch");
  assert.equal(summary.blocking, true);
});
...
test("alert test evidence summary blocks physical raw text mismatch", () => {
  const mismatchedTextSignal = normalizeBridgeSignalEvent({
    event_id: "bus-raw-text-mismatch",
    event_type: "signal.observed",
    source_bot: "chrome-discord-bridge",
    created_at: "2026-06-27T17:00:00.000Z",
    correlation_id: "chrome-message-raw-text-mismatch",
    payload: {
      contract_version: CHROME_DISCORD_MESSAGE_CONTRACT_VERSION,
      event_id: "chrome-message-raw-text-mismatch",
      channel_id: "chrome-alerts",
      channel_name: "chrome-alerts",
      author_id: "mike",
      author_name: "MikeInvesting",
      ...
      ingestion_result: {
        status: "accepted",
        alert_inserted: true,
        alert_id: "alert-raw-text-mismatch",
        trade_requested: true,
        trade_request_reason: "risk approved; order intent queued",
        skip_reason: "",
      },
  ...
  const auditedDecision = normalizeBridgeAlertDecisionEvent({
    ...
    details: {
      ...
      channel: { id: "chrome-alerts", name: "chrome-alerts" },
      author: { id: "mike", name: "MikeInvesting" },
      raw_text: "BTO QQQ 400C 6/21 @ 1.25",
      parsed: { ticker: "QQQ" },
      parser: { confidence: "high" },
      source: {
        key: "chrome-alerts",
  ...
  const chain = buildAlertEvidenceChains({ signals: [mismatchedTextSignal], decisions: [auditedDecision] })[0];
  const summary = buildAlertTestEvidenceSummary(chain);
  ...
  assert.equal(summary.queueLabel, "Order request queued");
  assert.equal(summary.auditLabel, "Audit audit-raw-text-mismatch");
  assert.equal(summary.blocking, true);
});

test("alert test evidence summary blocks physical proof with missing raw text", () => {
  const variants = [
    {
      name: "missing signal raw text",
      signalRawText: "",
      decisionRawText: "BTO SPY 500C 6/21 @ 1.25",
    },
    {
      name: "missing audited raw text",
      signalRawText: "BTO SPY 500C 6/21 @ 1.25",
      decisionRawText: "",
    },
  ];
  ...
  for (const variant of variants) {
    const physicalSignal = normalizeBridgeSignalEvent({
      ...
      source_bot: "chrome-discord-bridge",
      created_at: "2026-06-27T17:00:00.000Z",
      correlation_id: `chrome-message-${variant.name.replaceAll(" ", "-")}`,
      payload: {
        contract_version: CHROME_DISCORD_MESSAGE_CONTRACT_VERSION,
        event_id: `chrome-message-${variant.name.replaceAll(" ", "-")}`,
        channel_id: "chrome-alerts",
        channel_name: "chrome-alerts",
        author_id: "mike",
        author_name: "MikeInvesting",
        raw_text: variant.signalRawText,
        parsed: { ticker: "SPY" },
        parser_metadata: { confidence: "high" },
        ingestion_result: {
          status: "accepted",
    ...
    const auditedDecision = normalizeBridgeAlertDecisionEvent({
      ...
      details: {
        ...
        channel: { id: "chrome-alerts", name: "chrome-alerts" },
        author: { id: "mike", name: "MikeInvesting" },
        raw_text: variant.decisionRawText,
        parsed: { ticker: "SPY" },
        parser: { confidence: "high" },
        source: {
          key: "chrome-alerts",
