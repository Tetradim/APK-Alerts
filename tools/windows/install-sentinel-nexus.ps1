...
[CmdletBinding()]
param(
    [string]$InstallRoot = "$env:LOCALAPPDATA\SentinelNexus",
    [string]$SentinelEchoRepoUrl = "https://github.com/Tetradim/Sentinel-Echo.git",
    [int]$ApiPort = 8003,
    [string]$TailscaleAuthKey = $env:TAILSCALE_AUTHKEY,
    [string]$MobileApiKey = $env:MOBILE_SENTINEL_ECHO_API_KEY,
...
function New-PairingDeepLink {
    ...
    return "sentinelnexus://pair?payload=$encoded"
}
...
function Ensure-SentinelEchoRepo {
    param([string]$Root, [string]$RepoUrl)

    $repoRoot = Join-Path $Root "Sentinel-Echo"
...
function Ensure-MobileFirewallRule {
    ...
    $ruleName = "Sentinel Nexus API $Port"
...
function Test-MobileApiPreflight {
    ...
    if (-not $localHealthOk -and -not $failureStage) {
        ...
        $repairHint = "Start the Sentinel Echo remote API on 0.0.0.0:$Port, then rerun Pairing Doctor from the phone."
    }
...
function Write-RemoteEnvironment {
    ...
    $envPath = Join-Path $RepoRoot ".env.sentinel-nexus"
    $lines = @(
        "MOBILE_SENTINEL_ECHO_API_KEY=$ApiKey",
        "SENTINEL_ECHO_API_HOST=0.0.0.0",
        "SENTINEL_ECHO_API_PORT=$Port",
        "SENTINEL_ECHO_MOBILE_PAIRING_ENABLED=true"
    )
...
function Write-SetupEvidence {
    ...
    $evidence = [ordered]@{
        ...
        sentinelEchoRepoReady = $RepoReady
...
$repoRoot = Ensure-SentinelEchoRepo -Root $InstallRoot -RepoUrl $SentinelEchoRepoUrl
...
Write-Host "Sentinel Echo repo: $repoRoot"
