...
- Embedded Python evaluation first for Phone Engine parity with Consolidation.
...
The APK-Alerts repo should be organized so app, engine, contracts, and tests can evolve together.
...
engine/
  python/                 # embedded Consolidation-compatible engine subset
...
- parse Discord alerts using Consolidation-compatible logic;
...
Remote Engine is the existing Windows Consolidation runtime by default. It remains the easiest setup path for basic users, using the current Windows PC launcher and local dashboard.
...
Evaluate embedded Python first because it offers the strongest parity with existing Consolidation behavior.
...
- Python/backend unit tests carried forward from Consolidation for parser, policy, risk, orders, fills, and reconciliation.
...
- APK-Alerts is a monorepo with clear app, engine, contracts, sync, and test boundaries.
...
- Existing Consolidation live-trading gates still apply.
