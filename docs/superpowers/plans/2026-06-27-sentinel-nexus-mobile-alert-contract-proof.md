# Sentinel Nexus Mobile Alert Contract Proof Implementation Plan
...
**Architecture:** Keep normalized bridge evidence in `@sentinel-nexus/contracts` as the source. Add a mobile summary gate that inspects the chain's signal/audit contract fields, exposes a `contractLabel`, and blocks test clearance when identity or contract proof is missing or wrong.
...
```powershell
git add docs/superpowers/plans/2026-06-27-sentinel-nexus-mobile-alert-contract-proof.md apps/mobile/src/state/alertEvidenceState.ts apps/mobile/src/state/alertEvidenceState.test.ts apps/mobile/src/screens/AlertsScreen.tsx
