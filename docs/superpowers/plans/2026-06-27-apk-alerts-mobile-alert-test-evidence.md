...
**Architecture:** APK-Alerts already mirrors bridge signal events and audited decision events from Consolidation. Mobile will derive a read-only test-evidence summary per alert chain that distinguishes physical bridge observation from silent audit-only proof, then reports parse, source, queue, audit, and capture evidence. This batch must not claim that a live physical test occurred unless the remote evidence contains bridge observation proof.
...
```bash
git add docs/superpowers/plans/2026-06-27-apk-alerts-mobile-alert-test-evidence.md apps/mobile/src/state/alertEvidenceState.ts apps/mobile/src/state/alertEvidenceState.test.ts apps/mobile/src/screens/AlertsScreen.tsx
