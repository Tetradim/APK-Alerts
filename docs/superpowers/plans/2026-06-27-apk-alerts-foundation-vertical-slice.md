# APK-Alerts Foundation Vertical Slice Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create the buildable APK-Alerts monorepo foundation with typed event/lease contracts, a local event-log implementation, and an Android operator cockpit shell.

**Architecture:** This is the first vertical slice of the approved APK-Alerts mobile engine design. It establishes workspaces, contract boundaries, lease/idempotency primitives, an in-memory event log, and a mobile app shell that starts from real "not paired/not active" state instead of sample trading data.
...
```json
{
  ...
  "scripts": {
    ...
    "mobile:dev": "npm --workspace @apk-alerts/mobile run dev",
    "mobile:android": "npm --workspace @apk-alerts/mobile run android",
    "mobile:web": "npm --workspace @apk-alerts/mobile run web"
  },
  "devDependencies": {
    "tsx": "^4.20.6",
    "typescript": "~5.9.3"
...
git commit -m "chore: initialize apk-alerts workspace"
```
...
```powershell
npm --workspace @apk-alerts/sync-client test
```
...
```powershell
npm --workspace @apk-alerts/sync-client run typecheck
```
