# Sentinel Nexus Mobile Alert Evidence Spine Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the mobile-side evidence spine that proves Chrome/Discord alerts are seen, parsed, policy-checked, audited, and surfaced in Sentinel Nexus without enabling live-money claims.

**Architecture:** Sentinel Nexus will mirror Sentinel Echo's existing Chrome bridge evidence contract instead of inventing a new protocol. Shared contracts will normalize bridge bus events, operator audit decisions, and bridge health fail-closed; the sync client will read Sentinel Echo's event endpoints; the Alerts tab will show the latest auditable alert chain from real remote data or an empty state.

**Tech Stack:** TypeScript, npm workspaces, Expo Router, React Native, Zustand, Node `tsx --test`, existing Sentinel Echo `/api/bus/events`, `/api/operator/events`, and `/api/discord/chrome-bridge/health` endpoints.
...
- Create `packages/contracts/src/bridgeEvidence.test.ts`
  - Tests accepted, skipped, duplicate, malformed, and stale bridge evidence normalization using fixtures derived from Sentinel Echo's existing tests.
...
- Update `README.md`
  - Documents that Alerts reads real Sentinel Echo evidence and does not simulate trades.
...
```powershell
npm --workspace @sentinel-nexus/contracts test -- src/bridgeEvidence.test.ts
```
...
```powershell
npm --workspace @sentinel-nexus/sync-client test -- src/remoteEvidenceClient.test.ts
```
...
```powershell
npm --workspace @sentinel-nexus/mobile test -- src/state/alertEvidenceState.test.ts
```
...
Document that the mobile Alerts tab consumes Sentinel Echo evidence from:
- `/api/bus/events`
- `/api/operator/events`
...
```powershell
git add README.md docs/superpowers/plans/2026-06-27-sentinel-nexus-mobile-alert-evidence-spine.md packages/contracts/src packages/sync-client/src apps/mobile/src apps/mobile/app/(tabs)/alerts.tsx
