# APK Alerts Peer Alert Endpoint Mobile Evidence Implementation Plan
...
**Tech Stack:** TypeScript, Node test runner, npm workspaces, shared `@apk-alerts/contracts`, Expo React Native, Zustand.
...
```powershell
npm --workspace @apk-alerts/peer-alert-server exec -- tsx --test src/peerAlertChallengeEndpoint.test.ts
```
...
```powershell
npm --workspace @apk-alerts/mobile exec -- tsx --test src/state/peerAlertFailsafeState.test.ts
```
...
```powershell
npm --workspace @apk-alerts/mobile exec -- tsx --test src/state/peerAlertFailsafeState.test.ts
npm --workspace @apk-alerts/mobile run typecheck
```
...
```powershell
git add docs/superpowers/plans/2026-06-27-apk-alerts-peer-alert-endpoint-mobile-evidence.md packages/peer-alert-server apps/mobile/src/state/peerAlertFailsafeState.ts apps/mobile/src/state/peerAlertFailsafeState.test.ts apps/mobile/src/screens/AlertsScreen.tsx
