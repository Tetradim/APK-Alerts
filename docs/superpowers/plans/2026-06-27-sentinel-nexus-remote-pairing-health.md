# Sentinel Nexus Remote Pairing Health Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the first real remote-engine connection slice so the Android app can store a Sentinel Echo API target, check `/api/health` and `/api/status`, and render actual phone/remote health without Lab, backtest, replay, fake trades, or demo data.

**Architecture:** Add typed normalization in `@sentinel-nexus/contracts`, a fetch-injected remote client in `@sentinel-nexus/sync-client`, and mobile state/view components in `apps/mobile`. The UI stays fail-closed: until a real remote check succeeds, it shows unpaired/offline state and does not imply execution authority.
...
- Create `packages/contracts/src/remoteHealth.ts`
  - Normalizes Sentinel Echo `/api/health` and `/api/status` payloads.
...
- Optional modify `README.md`
  - Add a short note that remote pairing currently checks Sentinel Echo `/api/health` and `/api/status`.
...
import assert from "node:assert/strict";
import test from "node:test";
import { buildRemoteEngineHealthSnapshot, normalizeRemoteHealthPayload, normalizeRemoteStatusPayload } from "@sentinel-nexus/contracts";
import {
...
```ts
import type { RemoteEngineHealthSnapshot } from "@sentinel-nexus/contracts";
import { buildRemoteEngineHealthSnapshot, normalizeRemoteHealthPayload, normalizeRemoteStatusPayload } from "@sentinel-nexus/contracts";
import { checkRemoteEngineHealth } from "@sentinel-nexus/sync-client";
...
export function EnginesScreen() {
  ...
  return (
    <ScreenFrame title="Engines" eyebrow="Sentinel Nexus">
      ...
      <View style={styles.panel}>
        <Text style={styles.panelTitle}>Remote Sentinel Echo API</Text>
        <TextInput
...
```md
- Engines tab can check a configured remote Sentinel Echo API through `/api/health` and `/api/status`; it fails closed until a real endpoint responds.
```
...
- There is no Lab tab.
- Engines tab renders `Remote Sentinel Echo API`, URL input, API key input, `Check Remote`, Phone card, Remote card.
